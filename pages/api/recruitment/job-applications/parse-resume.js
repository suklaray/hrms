...
export default async function handler(req, res) { 
  ...
  const jobDescriptionIdRaw = Array.isArray(fields.job_description_id) ? fields.job_description_id[0] : fields.job_description_id; 
  const jobDescriptionId = Number(jobDescriptionIdRaw); 
 
  if (!Number.isInteger(jobDescriptionId)) { 
    return res.status(400).json({ success: false, error: 'A valid job description is required.' }); 
  } 
  ...
  if (!jobDescription) { 
    return res.status(404).json({ success: false, error: 'Selected job description was not found.' }); 
  } 
  ...
  try { 
    const jobForScoring = { 
      ...jobDescription, 
      required_skills: asList(jobDescription.required_skills), 
      preferred_skills: asList(jobDescription.preferred_skills), 
    }; 
    ...
    return res.status(502).json({ success: false, error: 'Candidate matching failed: ' + err.message }); 
  } 
  ...
  return res.status(200).json({ success: true, data: parsed, recordId: record.id, matchingScore: matchingScore, jobDescription: { id: jobDescription.id, title: jobDescription.title } }); 
}
...
export default function CandidateForm() {
  ...
  useEffect(() => {
    ...
    const token = id; // In your case, "id" is the token from the URL
    ...
    const verifyToken = async () => {
      try {
        ...
        // If admin is viewing a submitted form, redirect to submitted page
        if (candidateData.form_submitted && candidateData.isAdmin) {
        ...
        console.log("Backend said:", status, errorMsg);
  ...
  useEffect(() => {
    ...
    const requiredFiles = requiredFileFields.filter(
      (field) => field !== "experience_certificate"
    );
    ...
    switch (name) {
      case "contact_no":
        if (!/^\d+$/.test(value)) {
          ...
          newErrors[
            name
          ] = `❌ Must be exactly 10 digits (current: ${value.length})`;
        } else {
          delete newErrors[name];
        }
        break;
      case "pincode":
        if (!/^\d+$/.test(value)) {
          ...
          newErrors[
            name
          ] = `❌ Must be exactly 6 digits (current: ${value.length})`;
        } else {
          delete newErrors[name];
        }
        break;
      case "aadhar_number":
        ...
        if (!/^\d+$/.test(cleanAadhar)) {
          ...
          newErrors[
            name
          ] = `❌ Must be exactly 12 digits (current: ${cleanAadhar.length})`;
        } else {
          delete newErrors[name];
        }
        break;
      case "pan_number":
        ...
        if (upperPan.length !== 10) {
          newErrors[
            name
          ] = `❌ Must be exactly 10 characters (current: ${upperPan.length})`;
      ...
      case "ifsc_code":
        ...
        if (upperIfsc.length !== 11) {
          newErrors[
            name
          ] = `❌ Must be exactly 11 characters (current: ${upperIfsc.length})`;
      ...
      case "account_number":
        if (!/^\d+$/.test(value)) {
          newErrors[name] =
            "❌ Account number must contain only digits. Example: 1234567890123";
        } else if (value.length < 9 || value.length > 18) {
          newErrors[
            name
          ] = `❌ Must be 9-18 digits (current: ${value.length}). Example: 1234567890123`;
        } else {
          delete newErrors[name];
        }
        break;
      case "account_holder_name":
        if (!/^[a-zA-Z\s]+$/.test(value)) {
          newErrors[name] =
            "❌ Name must contain only letters and spaces. Example: John Doe";
        } else if (value.length < 2) {
          newErrors[name] =
            "❌ Name must be at least 2 characters. Example: John Doe";
        } else {
          delete newErrors[name];
        }
        break;
      case "bank_name":
        if (!/^[a-zA-Z\s]+$/.test(value)) {
          newErrors[name] =
            "❌ Bank name must contain only letters and spaces. Example: State Bank of India";
        } else if (value.length < 3) {
          newErrors[name] =
            "❌ Bank name must be at least 3 characters. Example: HDFC Bank";
        } else {
          delete newErrors[name];
        }
        break;
      case "branch_name":
        if (!/^[a-zA-Z0-9\s,.-]+$/.test(value)) {
          newErrors[name] =
            "❌ Invalid branch name format. Example: Main Branch, Mumbai";
        } else if (value.length < 3) {
          newErrors[name] =
            "❌ Branch name must be at least 3 characters. Example: Main Branch";
        } else if (!/[a-zA-Z]/.test(value)) {
          newErrors[name] =
            "❌ Branch name must contain at least one letter. Example: Main Branch, Mumbai";
        } else {
          delete newErrors[name];
        }
        break;
    ...
    if (type === "file") {
      ...
      if (file) {
        ...
        if (name === "profile_photo") {
          ...
          errorMessage =
            "❌ Only JPG, PNG, and JPEG files are allowed for profile photo";
        } else {
          allowedTypes = [
            "image/jpeg",
            "image/png",
  ...
  const handleDocumentUpload = async (e, type) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setErrors((prev) => ({
        ...prev,
        [type]: "Only JPG, PNG, and PDF files are allowed",
      }));
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.extractedText) {
        ...
        if (number) {
          ...
          validateField(
            type === "aadhar_card" ? "aadhar_number" : "pan_number",
            number
          );
        } else {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^\d{12}$/.test(formData.aadhar_number.replace(/\s/g, ""))
    ) {
    ...
    if (
      formData.pan_number &&
      !/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(formData.pan_number.toUpperCase())
    ) {
            ...
            >
                  ...
                  <div>
                    ...
                    {candidate?.contact_number && (
                      <p className="text-green-600 text-xs mt-1">
                        ✓ Contact number loaded from candidate profile. You can
                        edit if needed.
                      </p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      onBlur={(e) =>
                        validateField("account_holder_name", e.target.value)
                      }
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
                        errors.bank_name
                          ? "border-red-500 bg-red-50"
                          : formData.bank_name && !errors.bank_name
                          ? "border-green-500 bg-green-50"
                          : "border-gray-300"
                      }`}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      onBlur={(e) =>
                        validateField("branch_name", e.target.value)
                      }
                      required
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
                        errors.ifsc_code
                          ? "border-red-500 bg-red-50"
                          : formData.ifsc_code && !errors.ifsc_code
                          ? "border-green-500 bg-green-50"
                          : "border-gray-300"
                      }`}
                    />
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  >
                    I have reviewed all the details provided above, and I
                    confirm that they are accurate and final.
                  </label>

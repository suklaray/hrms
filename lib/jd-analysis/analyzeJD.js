  ...
  try {
    ...
    for (let attempt = 0; attempt <= MAX_GEMINI_RETRIES; attempt += 1) {
      try {
        ...
        const canRetry = TRANSIENT_GEMINI_STATUSES.has(status) && attempt < MAX_GEMINI_RETRIES;
    ...
    const analysisError = new Error("Failed to analyze Job Description with Gemini");
    analysisError.status = Number(error?.status) || 500;
    throw analysisError;
  }
}

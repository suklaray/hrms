...
export async function verifyHRToken(req) {
  try {
    ...
    if (["hr", "admin", "superadmin"].includes(decoded.role)) {
      ...
      const userType = 'admin'; // HR/Admin/SuperAdmin all use admin sessions
      ...
      if (!session) {
        console.log(`No session found for ${decoded.role} ${userId} during token verification`);
        return null;
      }
      ...
      if (timeSinceLastActivity > SESSION_TIMEOUT) {
        console.log(`Session expired for ${decoded.role} ${userId}. Inactive for ${Math.round(timeSinceLastActivity / 1000)} seconds (limit: ${SESSION_TIMEOUT / 1000}s)`);
      ...
      if (!shouldIgnoreActivity) {
        ...
        console.log(`[SESSION REFRESH] ${decoded.role} ${userId} session activity updated via verifyHRToken`);
      } else {
        console.log(`[SESSION SKIPPED] ${decoded.role} ${userId} activity ignored for ${req.url}`);
      }

...
function matchIntent(features, patterns = INTENT_PATTERNS) {
  ...
  // Find best match
  for (const [intent, data] of Object.entries(intentScores) as [string, any][]) {
  ...
  try {
    ...
    try {
      ...
      if (similarQuestions.length > 0) {
        ...
        const learnedConfidence = parseFloat(String(bestSimilar.confidence_score)) || 0;
...
export function handleConfirmationResponse(userId: any, isPositive: any, role: any = 'employee') {
  const context = getConversationContext(userId);
  if (!context?.pendingConfirmation) return null;
...
export async function getLearningInsights() {
  try {
    ...
    const confidenceByIntent: Record<string, number[]> = {};
    const records: any[] = Array.isArray(dbData) ? dbData : (dbData as any)?.insights || [];
    ...
    const avgConfidenceByIntent: Record<string, number> = {};
    (Object.entries(confidenceByIntent) as [string, number[]][]).forEach(([intent, scores]) => {
      avgConfidenceByIntent[intent] =
        scores.reduce((a, b) => a + b, 0) / scores.length;
    });

    return {
      totalQuestions: records.reduce((sum: number, record: any) => sum + record.frequency, 0),
      activeConversations: conversationContext.size,
      mostCommonIntents: Object.entries(intentStats)
        .sort(([, a], [, b]) => Number(b) - Number(a))
        .slice(0, 5),
      avgConfidenceByIntent,
      recentQuestions: records.slice(0, 10).map((record: any) => ({

  ...
  try {
    ...
    recentLeaveUpdates.forEach((leave) => {
      notifications.push({
        ...
        borderColor:
          leave.status === "Approved" ? "border-green-300/80" : "border-red-300/80",
        autoDismiss: false,
    ...
    const recentHolidays = await prisma.calendar_events.findMany({
      where: {
        ...
        event_date: { gte: now },
        created_at: { gte: oneHourAgo },
        OR: [{ visible_to: "all" }, { visible_to: userId }],
      },

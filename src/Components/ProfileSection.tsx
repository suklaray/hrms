  ...
  useEffect(() => {
    ...
    if (isWorking && workStartTime) {
      interval = setInterval(() => {
        const now = new Date();
        const diff = Number(now) - Number(workStartTime);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        <div className="flex items-center space-x-4">
              ...
              <>
                <Image
                  ...
                  onError={(e: any) => {
                    ...
                    if (target?.nextSibling?.style) target.nextSibling.style.display = 'flex';
                  }}
                />
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-lg font-semibold text-gray-900 truncate" title={user?.name || "N/A"}>
              {user?.name || "N/A"}
            </h3>
